...
public class placeMaxKnightsInNxNBoard {
    ...
    public static void placeKnight(char board[][], int row, int col, int num) {
        int n = board.length;
        if (row == n) {
            if (num == maxNum) {
                for (int i = 0; i < n; i++) {
            ...
            board[row][col] = 'k';
            if (col < n - 1) placeKnight(board, row, col + 1, num + 1);
            else placeKnight(board, row + 1, 0, num + 1);

            // backtracking
            board[row][col] = '.';
        }
        if (col < n - 1) placeKnight(board, row, col + 1, num);
        else placeKnight(board, row + 1, 0, num);
